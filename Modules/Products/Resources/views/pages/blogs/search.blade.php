        ...
        <div class="row justify-content-center flex-wrap-reverse">
            <div class="col-lg-8 mt-3 col-sm-12">
                <div class="bg-white shadow-sm rounded-3 p-4">
                    <h3 class="text-brown fw-bold">Bài viết nổi bật</h3>
                    <div class="mb-3 fs-14px">Tổng hợp các bài viết chia sẻ về kinh nghiệm tự học lập trình online và
                    ...
                    @endif
{{--                    @include("base::layouts.includes._inc_empty_items", ["items" => $items])--}}
                </div>
            </div>
            <div class="col-lg-4 mt-3 col-sm-12 ">
                <div class="p-4 bg-white rounded-3 shadow-sm">
                    <h4 class="fw-bold">Chuyên mục</h4>
                    <ul class="mt-3">
                        @foreach($blogCategories as $category)
                            <li class="mb-1" style="list-style-type: none">
                                <a href="{{ route("get.blog.search", ["cate" => $category["id"]]) }}" class="link-dark {{ form_query("cate", $query) == $category["id"] ? "fw-bold" : "" }} " href="">{{ $category["name"] }}</a>
                            </li>
                            @if(isset($category["child"]) && count($category["child"]))
                                @foreach($category["child"] as $child)
                                    <li class="ms-5 mb-1">
                                        <a class="link-dark {{ form_query("cate", $query) == $child["id"] ? "fw-bold" : "" }}" href="{{ route("get.blog.index", ["cate" => $child["id"]]) }}">{{ $child["name"] }}</a>
                                    </li>
                                @endforeach
                                @endif
                        @endforeach
